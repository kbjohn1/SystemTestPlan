...
/**
 * User Interface for the SystemTestPlan project that provides the user the
 * ability to interact with test plans, tests, and their results.
 * 
...
public class SystemTestPlanGUI extends JFrame implements ActionListener {
		...
		if (e.getSource() == itemClear) {
			if (manager.isChanged()) {
				int select = JOptionPane.showConfirmDialog(null,
						"Test plans are unsaved. Would you like to save before clearning?", "Warning",
	...
	private class TestPlansPanel extends JPanel implements ActionListener {
		...
		/**
		 * Creates the test plan list and populates the tests for the current test plan.
		 */
		...
		/**
		 * Handles actions when the user interacts with buttons and items on the JPanel.
		 */
		@Override
		public void actionPerformed(ActionEvent e) {
			...
			if (e.getSource() == comboTestPlans) {
				...
				try {
					String testPlanName = (String) JOptionPane.showInputDialog(this, "Test Plan Name?",
							"Create New Test Plan", JOptionPane.QUESTION_MESSAGE);
				...
				try {
					if (manager.getCurrentTestPlan() instanceof FailingTestList) {
						JOptionPane.showMessageDialog(SystemTestPlanGUI.this,
								"The Failing Tests list may not be edited.");
					} else {
						String testPlanName = (String) JOptionPane.showInputDialog(this, "Update Test Plan Name?",
								"Edit Test Plan", JOptionPane.QUESTION_MESSAGE, null, null,
								manager.getCurrentTestPlan().getTestPlanName());
				...
				try {
					if (manager.getCurrentTestPlan() instanceof FailingTestList) {
						JOptionPane.showMessageDialog(SystemTestPlanGUI.this,
								"The Failing Test list may not be deleted.");
					} else {
		...
		private class TestCaseTableModel extends AbstractTableModel {
			...
			/**
			 * Constructs the TestCaseTableModel by requesting the latest information from
			 * the Manager.
			 */
			...
			/**
			 * Updates the given model with test cases information from the current test
			 * plan.
			 */
	...
	/**
	 * Inner class that creates the look and behavior for interacting with a
	 * TestCase.
	 * 
	 * @author Dr. Sarah Heckman
	 */
	private class TestCasePanel extends JPanel implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (e.getSource() == btnAdd) {
				try {
					if (idx == -1) {
						TestCase t = new TestCase(txtTestCaseId.getText(), txtTestType.getText(),
								txtDescription.getText(), txtExpectedResults.getText());
